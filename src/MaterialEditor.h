...
extern ArrowOverlayElement* _arrow;
extern MaterialPtr _material;

// Some global node data
extern SceneNode* mRedYellowLightsNode;
extern SceneNode* mGreenBlueLightsNode;
extern std::vector<AnimationState*> mAnimStateList;

extern GlyphBlock* _blk1;
extern Vector2 _blk1pos;

/** This class turns lights & billboards linked to them on & off */
class MaterialLightFlasher : public ControllerValue<Real>
{
protected:
	Light* mLight;
	...
	Real intensity;
public:
	MaterialLightFlasher(Light* light, Billboard* billboard, ColourValue maxColour)
	{
		mLight = light;
...
class MaterialLightFlasherControllerFunction : public WaveformControllerFunction
{
public:
	MaterialLightFlasherControllerFunction(WaveformType wavetype, Real frequency, Real phase) : WaveformControllerFunction(wavetype, 0, frequency, phase, 1, true)
	{
...
class MaterialLightingListener : public ExampleFrameListener
{
protected:
public:
    MaterialLightingListener(RenderWindow* win, Camera* cam)
...
/** Application class */
class MaterialEditorApplication : public ExampleApplication
{
protected:
...
//		olc->setBorderMaterialName(head->getSubEntity(0)->getMaterialName());
		...
		mRedLightFlasher = ControllerValueRealPtr(
            new MaterialLightFlasher(mRedLight, mRedLightBoard, ColourValue::Red));
		mBlueLightFlasher = ControllerValueRealPtr(
            new MaterialLightFlasher(mBlueLight, mBlueLightBoard, ColourValue::Blue));
		mYellowLightFlasher = ControllerValueRealPtr(
            new MaterialLightFlasher(mYellowLight, mYellowLightBoard, ColourValue(1.0, 1.0, 0.0)));
		mGreenLightFlasher = ControllerValueRealPtr(
            new MaterialLightFlasher(mGreenLight, mGreenLightBoard, ColourValue::Green));
	...
	void createFrameListener(void)
    {
        ...
        mFrameListener= new MaterialLightingListener(mWindow, mCamera);
